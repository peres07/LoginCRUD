...
export const AuthProvider = ({ children }) => {
  ...
  async function register(email, username, password, code) {
    try {
      const response = await registerRequest(email, username, password, code);
      return response;
    } catch (error) {
      //pass
    }
  ...
  async function changeUsername(username, password, code) {
    try {
      const changeResponse = await changeUsernameRequest(username, password, code);
  ...
  async function changeEmail(username, password, code) {
    try {
      const changeResponse = await changeEmailRequest(username, password, code);
      if (changeResponse.status === 200) {
        logout();
      }
    } catch (error) {
  ...
  async function changePassword(newPassword, confirmNewPassword, password, code) {
    try {
